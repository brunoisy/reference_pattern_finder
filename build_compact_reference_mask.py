...
def build_compact_global_mask(refs):
    ...
    alignment = global_pairwise_align(CustomSequence(masks[0]), CustomSequence(masks[1]), gap_open_penalty=1, gap_extend_penalty=0, substitution_matrix=subst_matrix)
    print(alignment)
...
def build_court_masks(court_id):
    ...
    (decision_ref_mask, alignment) = build_compact_global_mask(decision_refs)
    # docket_ref_mask = build_global_mask(docket_refs)

    return (decision_ref_mask, docket_ref_mask, alignment)
...
# PRV : 285
(decision_ref_mask, docket_ref_mask, alignment) = build_court_masks(285)
